...
///////////////////////
DiffuseSpecularMapRenderer::DiffuseSpecularMapRenderer(float width, float height)
	: Renderer(width, height), y_rot_(0)
{
	Init();
}

DiffuseSpecularMapRenderer::~DiffuseSpecularMapRenderer()
{
	diffuse_map_->Deinit();
...
bool DiffuseSpecularMapRenderer::Init()
{
	//쉐이더 
	...
	SR_ASSERT(diffuse_map_init_result == true);
	...
	SR_ASSERT(specular_map_init_result == true);

	return true;
}

bool DiffuseSpecularMapRenderer::Update(float dt)
{
	y_rot_ += 2.0f * dt;
	bool running = !glfwGetKey(GLFW_KEY_ESC) && glfwGetWindowParam(GLFW_OPENED);
	return running;
...
void DiffuseSpecularMapRenderer::Draw()
{
	//sample mesh
	//haruna::SolidCubeFactory cube_factory(1, 1, 1);
	//auto data = cube_factory.CreateNormalMesh();
...
ToonRenderer::ToonRenderer(float width, float height)
	: Renderer(width, height), y_rot_(0)
{
	Init();
}
...
bool ToonRenderer::Update(float dt)
{
	y_rot_ += 2.0f * dt;
	bool running = !glfwGetKey(GLFW_KEY_ESC) && glfwGetWindowParam(GLFW_OPENED);
	return running;
}
void ToonRenderer::Draw()
{
	//sample mesh
	//haruna::SolidCubeFactory cube_factory(1, 1, 1);
	//auto data = cube_factory.CreateNormalMesh();
