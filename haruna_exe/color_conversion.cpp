...
{
	std::string fs_path = Filesystem::GetAppPath("shader/gray_scale.fs");
	std::string vs_path = Filesystem::GetAppPath("shader/gray_scale.vs");
	ReadonlyCFile fs_file(fs_path);
	ReadonlyCFile vs_file(vs_path);
	if(!fs_file.Open()) {
		return false;
...
{
	std::string fs_path = Filesystem::GetAppPath("shader/sepia.fs");
	std::string vs_path = Filesystem::GetAppPath("shader/sepia.vs");
	ReadonlyCFile fs_file(fs_path);
	ReadonlyCFile vs_file(vs_path);
	if(!fs_file.Open()) {
		return false;
